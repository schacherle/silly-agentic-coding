...
6. **Prompt Maintenance**: Never edit compiled agent files inside [agents/](./agents) directly. Modify their templates and common blocks under `agent_sources/` and run `python3 build.py`. Ensure that `python3 build.py --check` passes before submitting edits.
