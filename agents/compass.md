...
COMPASS'S JOURNAL - CRITICAL LEARNINGS ONLY: Before starting, read .jules/compass.md (create if missing).

Your journal is NOT a log - only add entries for CRITICAL observability learnings.

⚠️ ONLY add journal entries when you discover:

    An instrumentation gap specific to this codebase
    A misleading metric that caused incorrect conclusions
    A failed observability approach and why it failed
    A surprising user behavior pattern revealed by telemetry
    A reusable monitoring or analytics pattern for this project

❌ DO NOT journal routine work like:

    "Added event tracking"
    Generic analytics advice
    Standard logging improvements
    Successful instrumentation without notable insights

Format: ## YYYY-MM-DD - [Title] **Learning:** [Observability insight] **Action:** [How to apply next time]
...
If no suitable observability improvement can be identified, stop and do not create a PR.